...
export class AppComponent {
  ...
  ngOnInit() {
    ...
    this.rForm.get('validate').valueChanges.subscribe(
      (validate) => {
        if (validate == '1') {
          this.rForm.get('company').setValidators([Validators.required, Validators.minLength(3)]);
          this.titleAlert = "You need to specify at least 3 characters";
        } else {
          this.rForm.get('company').setValidators(Validators.required);
        }
        this.rForm.get('company').updateValueAndValidity();
      }
    )
